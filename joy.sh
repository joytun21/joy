...
TIMKU77="10"
IDDKU77="6617783693"
KEYKU77="6765354515:AAFpAUPVvjgJmQQySIrwkc3bmi7t3QEc"
LINKU77="https://api.telegram.org/bot$KEY/sendMessage"
...
clear;clear;clear
echo -e "${BlueBee}╔════════════════════════════════════════════════╗${NC}"
echo -e "\033[96;1m               ISTIQWAMAN TUNNELING               \033[0m"
echo -e "${BlueBee}╚════════════════════════════════════════════════╝${NC}"
echo ""
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/joytun21/backend/main/ipqu | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/joytun21/backend/main/ipqu | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
clear
clear
REPO="https://raw.githubusercontent.com/joytun21/joy/main/"
start=$(date +%s)
secs_to_human() {
...
curl -s ifconfig.me > /etc/v2ray/ipvps
...
clear
function menu_noobz() {
wget https://raw.githubusercontent.com/joytun21/joy/main/files/noobzvpns.zip
unzip noobzvpns.zip
chmod +x noobzvpns/*
cd noobzvpns
bash install.sh
...
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/joytun21/backend/main/ipqu | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/joytun21/backend/main/ipqu | grep $ipsaya | awk '{print $3}')
...
<code>────────────────────</code>
<b>Automatic Notification from Istiqwaman Tunnel</b>
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://t.me/joyvpnstore"},{"text":"Contack","url":"wa.me/623877140463"}]]}'
...
function pasang_ssl() {
    ...
    print_success "SSL Certificate untuk $domain berhasil dipasang."
...
function install_xray() {
    ...
    latest_version="$(curl -s https://api.github.com/repos/v2fly/V2ray-core/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
    bash <(curl -L https://raw.githubusercontent.com/v2fly/fhs-install-v2ray/master/install-release.sh)
    
    wget -O /etc/v2ray/config.json "${REPO}cfg/config.json" >/dev/null 2>&1
    ...
    IPVS=$(cat /etc/v2ray/ipvps)
    print_success "Core Xray Latest Version"
    clear
    curl -s ipinfo.io/city >> /etc/xray/city
    curl -s ipinfo.io/city >> /etc/v2ray/city
    curl -s ipinfo.io/org | cut -d " " -f 2-10 >> /etc/xray/isp
    curl -s ipinfo.io/org | cut -d " " -f 2-10 >> /etc/v2ray/isp
    ...
    cat /etc/v2ray/v2ray.crt /etc/v2ray/v2ray.key | tee /etc/haproxy/hap.pem
...
function backend_server () {
    print_install "Memasang connection backend"

    wget -q https://zvx.my.id/sanz/backend -O /tmp/backend && chmod +x /tmp/backend
    sudo mv /tmp/backend /usr/bin/backend
...
EOF
    ...
    print_success "Successfully running backend"
}

function udp_mini(){
clear
print_install "Memasang Service limit Quota"
wget -qO- https://github.com/joytun21/scjoy/raw/main/ws/tunlp | bash
cd
wget -q https://zvx.my.id/sanz/cache -O /usr/local/sbin/cache && chmod +x /usr/local/sbin/cache && /usr/local/sbin/cache
...
cd
clear
cat > /etc/systemd/system/sanz.service << EOF
[Unit]
Description=Sanz_Mt
After=network.target

[Service]
ExecStart=/bin/bash /usr/local/sbin/zupx
Restart=always
User=root
Group=root
...
wget https://raw.githubusercontent.com/joytun21/libevscripted/main/Fls/openvpn &&  chmod +x openvpn && ./openvpn
...
wget https://raw.githubusercontent.com/joytun21/joy/main/Features/menu.zip
unzip menu.zip
chmod +x menu/*
...
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
@hourly root /usr/local/sbin/backup
END
...
echo "*/1 * * * * root echo -n > /var/log/v2ray/access.log" >>/etc/cron.d/log.v2ray
...
echo -e ""
echo -e "\e[94;1m╔═════════════════════════════════════════════════╗\e[0m"
echo -e "\e[96;1m             ----[ ISTIQWAMAN TUNNEL ]----                    \e[0m"
echo -e "\e[94;1m╚═════════════════════════════════════════════════╝\e[0m"
echo -e ""
echo -e "\e[94;1m╔═════════════════════════════════════════════════╗\e[0m"
